...
!сейф [вкл/выкл] [пинг игрока] - включение и выключение возможности использовать сейф`;
          break;
        case 5:
...
-ДМ [количество] [пинг игрока](для админов) - потратить драконьи монеты`;
        ...
        let player = main.Players[message.senderId];
          ...
          if (!player.safeAvailable && !main.isGod(message.senderId))
            return message.send('Нет доступа к сейфу');
        msg = `!сейф [пинг игрока](для админов) - просмотр содержимого сейфа

!сейф положить [номер предмета] [пинг игрока](для админов) - перенести предмет из своего инвентаря во внешнее хранилище

!сейф взять [номер предмета] [пинг игрока](для админов) - забрать предмет из внешнего хранилища в свой инвентарь`
        break;
        default:
